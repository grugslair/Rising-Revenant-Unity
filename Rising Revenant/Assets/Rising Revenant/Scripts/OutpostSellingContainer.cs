...
public class OutpostSellingContainer : MonoBehaviour
{
    ...
    public async void PurchaseTrade()
    {
        if (!owner)
        {
            ...
            var endpoint = new DojoCallsManager.EndpointDojoCallStruct { account = DojoEntitiesDataManager.currentAccount, addressOfSystem = DojoCallsManager.tradeOutpostActionsAddress, functionName = "purchase" };
